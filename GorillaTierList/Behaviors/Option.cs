...
namespace GorillaTierList.Behaviors
{
    public class Option : MonoBehaviour
    {
        ...
        internal void Update()
        {
            if (refObject != null)
            {
                Transform nearest = Main.Instance.GetNearest(IsInLeftHand);
                Transform target =
                        nearest != null && Vector3.Distance(GTPlayer.Instance.bodyCollider.transform.position,
                                Main.Instance.tierObject.transform.position) <= 8f
                                ? Vector3.Distance(GTPlayer.Instance.bodyCollider.transform.position,
                                          Main.Instance.ReturnDropper.transform.position) <= 1.6f
            ...
            bool shouldBeInLeftHand = leftHandDown &&
                                      Vector3.Distance(GTPlayer.Instance.LeftHand.controllerTransform.transform.position,
            ...
            if (!InHand && !Main.Instance.InLeftHand && shouldBeInLeftHand)
            {
                ...
                if (refObject == null)
                {
                    ...
                    refObject.GetComponentInChildren<Text>().text =
                            gameObject.transform.GetComponentInChildren<Text>().text;
                }
            ...
            bool shouldBeInRightHand = rightHandDown &&
                                       Vector3.Distance(GTPlayer.Instance.RightHand.controllerTransform.transform.position,
                                               gameObject.transform.position) < 0.2f;
            ...
            if (InHand)
            {
                ...
                if (IsInLeftHand && !leftHandDown)
                {
                    ...
                    if (refObject.transform.parent.childCount >= 13 &&
                        refObject.transform.parent            != Main.Instance.DropperCanvases[0].transform)
                    {
                        InHand = false;
                ...
                if (Vector3.Distance(GTPlayer.Instance.bodyCollider.transform.position,
                            Main.Instance.tierObject.transform.position) >= 5)
                {
                    if (refObject == null) return;
